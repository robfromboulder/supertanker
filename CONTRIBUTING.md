...
* Scan the container image: `trivy image supertanker:7.0.0`
...
* Your local builds will always be versioned `7.0.0` (and this is assumed by build scripts)
* Public release numbers use the installed Graylog version and a patch version like this: `7.0.beta1` (for 1st beta) or `7.0.0c` (for 3rd build of 7.0.0) 
...
```bash
./release.sh 7.0.(BUILD_NUMBER)(BUILD_LETTER)
```
...
Add release tag:
```bash
git tag 7.0.(BUILD_NUMBER)(BUILD_LETTER)
git push origin v7.0.x --tags
```
